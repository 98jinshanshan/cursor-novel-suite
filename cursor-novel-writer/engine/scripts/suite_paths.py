#!/usr/bin/env python3
"""Discover Novel Suite monorepo root — delegates to novel_suite.core.paths when installed."""
...
try:
    ...
    ensure_src_path()
    from novel_suite.core.paths import *  # noqa: F403
except ImportError:
    import os
    ...
    def suite_root(*, start: Path | None = None) -> Path:
        ...
        if env:
            ...
            if is_suite_root(candidate):
                return candidate
            raise SystemExit(
                f"ERROR: {ENV_ROOT}={env} is not a valid suite root (need {MARKER} + {WRITER_CLI})"
            )
