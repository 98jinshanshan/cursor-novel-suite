# 写作计划 — 雾港来信 demo

## Pipeline 进度（novel-pipeline 门控）

- [x] Phase 0: 选品（novel-market-scan → canon/concept-brief.md）
- [x] Phase 1: 立项（story-init）
- [x] Phase 2: 世界观 + 人物（worldbuilding + character-management）
- [x] Phase 3: 大纲（plot-structure）
- [x] Phase 4: 文风契约（canon/voice-brief.md）
- [x] Phase 5: 逐章写作（chapter-writing）— 第1章试章
- [ ] Phase 6: 验证（novel-review — blocker 清零）
- [ ] Phase 7: 去 AI 味（Sable + deai-checklist）
- [ ] Phase 8: 再验证（re-review，最多 2 轮）
- [ ] Phase 9: 导出（novel-export）

## 章节进度

| 章 | 标题 | 状态 | 字数 |
| --- | --- | --- | --- |
| 01 | 试章 | draft | ~800 |

## 章节计划

- [x] 第01章：试章 — 匿名信出现
- [ ] 第02章：待写

## 备注

- 验证报告：`reviews/ch01-review.md`（示例）
- 文风契约：`canon/voice-brief.md`
